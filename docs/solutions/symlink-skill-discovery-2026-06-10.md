...
**CC 发现行为：已确认（2026-06-10，用户实测）。**
- 用户在新 CC 会话的 slash 菜单里看到了 `/sm-spike-demo`——该 skill 的目录是指向 `/tmp` 的软链，CC 正常发现并把它列为可用 skill。**软链跟随成立，整个 symlink-into-live-mirror 模型的命脉验证通过。**
- 印证团队既有 solution `native-plugin-install-strategy`（Superpowers 模式）的实证，并满足其关键约束：skill 必须是 skills 根的**直接子项**——SkillManage 的 `<root>/<name> → 仓/<skill-dir>` 正满足。
- spike 用的测试 skill（`~/.claude/skills/sm-spike-demo` + `/tmp/sm-spike-src`）验证后已清理。
