...
## 六之三、UI 原则：链视图 vs 源视图（2026-06-18 记录）

**「现状/inventory 列表」里展示的条目（本地手写真身除外）都是软链**——它们指向别处的本体（git 镜像 / `~/.agents/skills` / `~/.skillmanage/local`）。由此定一条 UI 原则：

- **链视图（现状/inventory）只操作「链」**：启用/停用（建/拆软链），**不对本体下手**（不卸载、不删真身）。
- **本体操作只属于「源」视图**：卸载 skills.sh 真身 → 在 skills.sh 源列表弹窗；删 git 仓 → 在 git 源卡；删本地 skill → 在本地源。
- **卸载与「自动同步」状态无关**：本体可卸载，即使该链正处于自动同步（卸载动的是本体，不是某条链的同步开关）。

落地：`openDetail(@agents)`（从 skills.sh 源弹窗/搜索进入）与 skills.sh 列表弹窗每卡 → 有「卸载」；`openDetailAt`（现状视图软链详情）→ 只读，无卸载。

## 七之零、最终结论（2026-06-18，推翻多源配置）

**不做「多源配置」。搜索就用 skills.sh `find` 本身。** 推理链：

- `find` 是一个托管索引，**本身已聚合所有被收录的仓**（一次 `find react` 返回 5+ 个不同 owner/repo）。
- 「配置/勾选源」能做的只是**对 find 已返回的结果做过滤**——而每条结果卡片**已显示来源 + 安装数**，用户肉眼即可判断。等于把看得见的再筛一遍，**装饰性、不值得做**。
- 「配置源」**只有一种情况有意义**：想搜的仓**不在 find 索引里** → 必须对该仓单独枚举（`-l` clone / GitHub API），又把成本带回来。而 find 已覆盖权威仓，此需求边缘。

→ **v5.0.0 已交付的在线搜索 + 安装，对「社区发现」需求已足够。** 多源是伪需求，封存。

**唯一可选的小增强**（属降噪，非多源架构）：若 find 结果杂牌作者过多，可加「安装数 ≥ N」或「仅某几个可信 owner」的轻过滤。可选，按需。

下面「七之二」是被本结论推翻前的方案，留作记录。
