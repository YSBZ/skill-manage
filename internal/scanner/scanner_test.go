...
// TestScanWrapperSkillDir verifies the <realname>/skill/SKILL.md layout (a
// generic "skill" wrapper dir) is named after its parent, not the wrapper — so
// such skills don't all collapse to (and collide as) "skill".
func TestScanWrapperSkillDir(t *testing.T) {
	root := t.TempDir()
	mkSkill(t, root, "knowledge/i18n/rtl/skill")           // → rtl
	mkSkill(t, root, "knowledge/analytics/faw/skill")      // → faw
	mkSkill(t, root, "knowledge/perf/optimization/skills") // → optimization (plural wrapper too)
	mkSkill(t, root, "normal-skill")                       // → normal-skill (unchanged)
	skills, err := Scan(root)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{"rtl", "faw", "optimization", "normal-skill"} {
		if !got[want] {
			t.Errorf("expected skill named %q, got %v", want, got)
		}
	}
	if got["skill"] || got["skills"] {
		t.Errorf("wrapper dir name must not be used as the skill name: %v", got)
	}
}
